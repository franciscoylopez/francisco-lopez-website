// Gate de refactor para los showcase: el HTML servido no puede cambiar (P37.69).
//
...
//     npm run gate:showcase -- save       # ANTES de tocar nada
//     …refactor…
//     npm run gate:showcase               # después: sale 0 si nada cambió
//
...
const BASELINE = join("scripts", ".showcase-baseline");
const ACTUAL = join("scripts", ".showcase-actual");

/** Las cuatro variantes: las dos páginas × los dos idiomas. */
const ROUTES = [
  "/brand-kit",
  "/design-system",
  "/en/brand-kit",
...
const slug = (route: string) => route.slice(1).replaceAll("/", "-") + ".html";
