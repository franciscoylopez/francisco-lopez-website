---
canonical: https://franciscolopez.es/en
lang: en
title: From discovery to data.
description: "Senior Product Manager with 10+ years in B2B and B2C SaaS, and one exit (TheTool → AppRadar). From discovery to data: I research, prototype, build and measure."
last-updated: 2026-08-17
---

Senior Product Manager · UX · SaaS · AI · Builder

# From discovery to data.

I research, prototype, build and measure.

![Portrait of Francisco López](https://franciscolopez.es/_next/image?url=%2Fimg%2Ffrancisco-hero-estudio-4x5.webp&w=3840&q=75)

Key moments

## Milestones

2019 - 2026 · 5 milestones

Name · Impact · Year

(01)

Emendu

Strategic partnership with Sesame HR.

2026

(02)

INDYA

Monthly churn **16% → 10%**, first-month activation **+28%**.

2023

(03)

INDYA

Selected for Apple App Store Foundations.

2022

(04)

TheTool

Acquired by AppRadar. · Exit

2021

(05)

TheTool

Nominated for Best ASO Software in Europe (App Promotion Summit).

2019

Process

## How I work

An end-to-end method: from the problem to the data that confirms the goal was met. Six stages that feed one another.

01

### Discovery

Interviews, data and business context to separate the real problem from the symptom before proposing a solution.

02

### UX

I turn findings into architecture and flows: what the user sees, in what order and why.

03

### Prototyping

Clickable prototypes to build shared understanding before committing development.

04

### Development

Functional definition and coordination with the technical team, from roadmap to release.

05

### Launch

Selective deployment: standard or feature flags, progressive rollout and A/B testing to assess whether the goal was met.

06

### Analytics

Instrumentation and metrics to measure real impact and feed the next discovery.

Beyond the PM

Founder several times. Exit once. From what didn't work out, I learned what no framework teaches.

Before that, years acquiring users; before measuring product, I understood how to attract them.

Journey

## Experience

Product

Ten years leading product in B2B and B2C SaaS, from founder with an exit to Product Manager at startups and scale-ups, with a constant focus on discovery, UX and measurable results.

[Download CV](https://franciscolopez.es/cv/francisco-lopez-cv-en.pdf)

Feb 2025 (2025-02) - Present

[Product Manager](https://franciscolopez.es/en/career/emendu)

Emendu

Turned a newly built SaaS into the company's business model: redefined the ICP, digitalized the operation and opened a channel with the Sesame HR partnership.

Feb 2024 (2024-02) - Dec 2024 (2024-12)

[Cofounder & Product](https://franciscolopez.es/en/career/kuotip)

KUOTIP

Validated review fraud with users and brands before building anything; defined the MVP with voice verification and AI, and supported the CEO in the pre-seed fundraising.

Jan 2022 (2022-01) - Dec 2023 (2023-12)

[Product Lead](https://franciscolopez.es/en/career/indya)

INDYA

Organised growth around the lifecycle rather than features: monthly churn from 16% to 10%, with activation, engagement and retention as the roadmap's levers.

Oct 2021 (2021-10) - Dec 2021 (2021-12)

[Product Manager](https://franciscolopez.es/en/career/freepik)

Freepik

Researched and defined features for the contributors area; redesigned sign-up and onboarding, where 75% dropped off, and owned the squad's OKRs.

2015 - 2021

Shutapp Projects

One venture · two roles

May 2016 (2016-05) - Oct 2021 (2021-10)

[Cofounder & Product](https://franciscolopez.es/en/career/thetool)

TheTool

Cofounder and product lead from zero to the sale: vision, MVP, roadmap and the development team, through to the acquisition by AppRadar in 2021.

Sep 2015 (2015-09) - Dec 2016 (2016-12)

COO

PICKASO

Professionalized the agency's structure and service portfolio; ran the market research on TheTool's future competitors.

Earlier experience · Marketing & Growth

This earlier stage in marketing and growth builds the analytical, experimentation and user-first foundation that defines my approach as a Product Manager.

Sep 2013 (2013-09) - Sep 2015 (2015-09)

Digital Marketing Manager

Ontecnia (Malavida, Lecturalia, BonViveur…)

Organic growth from 3.2M to 9.4M monthly visits; shifted the business model from intrusive installers to valuable content and video monetization: the start of my turn toward product-first.

2009 - 2013

Digital Marketing / Performance

Havas Media · Increnta · Miss Conversion

Acquisition and performance at leading agencies: the analytics, CRO, UX and leadership foundation that enabled the jump to product.

Tools

## Toolkit

Tools are means, not ends. These are some that have helped me do my job better and hit my goals.

Users

Management & Documentation

Design & prototyping

Development

Users

Amplitude

I define funnels and cohorts to see where and why users drop off.

Google Analytics

Acquisition and aggregate behavior at the top of the funnel.

Microsoft Clarity

Heatmaps and recordings to see the real friction on screen.

Typeform

Surveys and interviews to validate hypotheses before building.

Management & Documentation

Jira

I organize roadmap and sprints, with traceability from discovery to delivery.

Notion

My source of truth for specs, decisions and living documentation.

Miro

I map user journeys and facilitate discovery and prioritization with the team.

Mermaid.js

I document flows as versionable diagrams alongside the specs.

Design & prototyping

Claude Design

I explore visual direction and generate clickable mockups very early.

Figma

A shared space with design where we align before building.

v0

I generate first UI versions to iterate fast on real ideas.

Development

Claude Code

I build tools, create prototypes and automate tasks.

VS Code

My code editor, linked to Claude Code.

Vercel

I deploy prototypes and previews to validate in minutes.

GitHub

I document development to make technical review easier.

Learning

## Education

A foundation in marketing and communication, updated with specific training in product and agile methodologies.

Product

### Product Management

TheHeroCamp · 2021

### Scrum & Agile Leadership

theUncoding · 2021

Marketing

### Master's in Digital Communication

Olea Europea · 2001

### Commercial Management & Marketing

ESIC · 2000

The next step

## Let's talk

If product sits at the core of your company and you're building genuinely great experiences for your users through discovery, applied AI and data, I think we'd be a good fit. Senior Product Manager with 10+ years in B2B and B2C SaaS, happy working remote or hybrid, based in Spain. Write to me and let's talk.

[Get in touch](https://franciscolopez.es/en/contact)
