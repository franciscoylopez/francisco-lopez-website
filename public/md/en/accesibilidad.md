...
The contrast census deliberately abstains on sixteen pairs: the eight pieces of text that sit over a photograph, measured in both themes. There the background is decided by the image and not by the colour system, so it takes reading the pixel that actually gets painted. They are now measured that way: twelve reach AAA and four land between AA and AAA, the worst at 4.67:1. All four are the same element: the menu links when the bar passes over a photo. They meet the floor and not the target, and whether the bar should be made less transparent is still open.
