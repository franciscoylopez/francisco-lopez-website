...
> **No des por buena ninguna cifra en prosa de las que vas a leer: reprodúcela.** Esta skill
> presumió un 35 % de reducción del arranque que era un 3,2 %.
...
| **El metro que aprueba sobre lista vacía** | Un verificador que no encuentra nada y calla, o que cuenta sus propias constantes | 7 · **1 viva** |
...
| **Arreglar la mitad que se abre** | Un arreglo real que resuelve el lado que PRODUCE el problema y deja intacto el que lo CONSUME | **9** |
...
| **El dato que persigue al techo** | El techo **no** se mueve y la medida vive pegada a él, porque retirar solo se dispara al cruzarlo. Firma: el objetivo no se ha cumplido **nunca** | **2** |
...
| **El disparador en el momento equivocado** | La regla tiene portador, pero su cadencia es más lenta que el daño: un aviso alto esperando al cierre del sprint | **2** |
...
- **Una bandeja de avisos no es una tasa.** Mide la ventana, y pártela por la fecha en que
  aterrizó el arreglo.
...
**Los umbrales vigentes, para el cierre siguiente** *(hoy = 2026-09-17, tras «Lanzamiento»)*:

| Indicador | Hoy | Verde | Rojo | Comando |
| :-- | :-- | :-- | :-- | :-- |
| **Runs de CI en rojo** | **2,5 %** (2/80) 🟢 | ≤ 5 % | ≥ 15 % | `gh run list --workflow=CI --json conclusion` |
| Variación neta de `General` por sprint | **±0** (2 → 2) 🟢 | ≤ 0 | ≥ +4 | `SELLO_GENERAL` de `check-tablero.ts` |
| Veces que se movió un techo en el ciclo | **0** de 4 🟢 | 0 | ≥ 2 | `check:contexto` (última sección) |
| Margen del presupuesto de contexto | **29** 🔴 | ≥ 400 | < 100 | `check:contexto` |
| Suma de skills a demanda | **holgura 2** 🟢 | ≤ techo | > techo | `check:contexto` |
| Verificación ÷ producto | **0,7376** 🔴 | ≤ 0,45 | > 0,55 | `check:contexto` (quinta sección) desde D205 |
| Verificación ÷ producto DEL SPRINT | **3,8 : 1** 🔴 | ≤ 1,5 | ≥ 3 | `git diff --shortstat <base> HEAD -- <área>` |
| Días máx. con un aviso alto abierto | **7** 🔴 | ≤ 2 | ≥ 5 | `gh api …/dependabot/alerts` |
| Consumidores viejos tras una D-entry que retira | **4** (D214) 🔴 | 0 | ≥ 2 | `git grep` del identificador retirado |
...
`scripts/contexto/documentos.ts`.*
