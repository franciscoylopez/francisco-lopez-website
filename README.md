...
- **Analítica y consentimiento**: Google Tag Manager + GA4 y **Microsoft Clarity**, con **Consent Mode v2** y banner de consentimiento granular (RGPD), con página propia de privacidad y cookies. Lo que el consentimiento gatea, medido y no supuesto (`D198`): **ninguna cookie** y **Clarity**, que no carga. Lo que **no** gatea: el contenedor de GTM carga con el consentimiento en «no» por defecto, y GA4 recibe un `page_view` por carga **sin cookies**, porque `analytics_storage: denied` gobierna el almacenamiento y no el envío. Y hay **dos cosas que miden sin preguntar por decisión escrita, las dos declaradas en `/cookies`**: el contador de consentimiento (`D169`), tres enteros que dicen qué fracción del tráfico ve la analítica y que no son dato personal; y **Vercel Web Analytics** (`D170`), que sí lo trata, porque detrás del consentimiento mediría lo mismo que GA4 y con el mismo sesgo. La segunda es una excepción al criterio propio de este sitio, no a la norma, y así está escrita. **Que el ping cookieless de GA4 sea un tercer caso que declarar, o que haya que cerrarle el gate, está sin decidir** y es tarea abierta.
...
La tabla los lista todos, y eso incluye los dos **regeneradores**: no comprueban una propiedad, rehacen un artefacto derivado y exigen que el diff quede vacío, así que su arreglo no es corregir código sino volver a generarlo. Se escribió pensando solo en los que comprueban y por eso los dos se quedaron fuera hasta el 2026-09-02. El **contrato** de cada uno —qué garantiza y qué deja fuera— vive en [`GATES.md`](./GATES.md), que no se lee entera: se busca la fila.
...
| `novedades` | Que un PR cambie el copy de una sección publicada y nadie lo lea antes de subirlo. No comprueba nada: **enumera** qué secciones del artículo y del Design System toca el diff contra `main`, y distingue el copy —que hay que leer— del sello movido, que casi nunca lo es. Es la mitad que faltaba de `check:articulo`, que en verde solo dice «el sello cuadra» |
...
| `check:marco` | Que una página nueva salga sin enlace de salto, sin su `h1`, sin breadcrumb o con la metadata de otra. Mide el HTML **prerenderizado**, no el código: los helpers son opt-in, y escribirse la metadata a mano compila igual. De paso resuelve las referencias `@id` del JSON-LD dos veces: contra todo el sitio, que ningún validador externo comprueba, y dentro de la propia página, que es lo único que ve uno (`D75`, `D87`). Y desde `D195`, que dos enlaces no se anuncien igual llevando a sitios distintos: un barrido a mano encontró dieciséis pares en cuatro patrones, y axe eso lo marca «needs review», o sea que no lo levanta |
| `md:verificar` | Que el markdown de las 28 variantes que se sirve a los agentes se quede viejo. Es un **regenerador**: reconvierte el `<main>` del prerender y exige que coincida con lo commiteado, en los dos sentidos —un `.md` que ya no corresponda a ninguna variante sobra, y la negociación lo seguiría sirviendo—, así que el arreglo es `npm run build && npm run md` (`D158`) |
...
| `check:capturas` | Que la portada del repo enseñe un sitio que ya no es ese. Son tres PNG commiteados y el repositorio es **público** desde el 2026-08-19. No se sella el render —una captura no es determinista, y el HTML de la home se mueve casi cada tanda sin que la imagen deje de ser correcta—: se transcribe cada frase que la imagen ENSEÑA y se contrasta con su fuente viva, dieciocho en total. **Fuera queda lo puramente visual**, que ahí no hay entrada barata que sellar (`D194`) |
...
| `check:agentes` | Que el sitio deje de cumplir lo que le **promete** a un agente. Mira en cuatro sitios porque la promesa ocurre en cuatro: `llms.txt` y el markdown de las 28 variantes, en el artefacto; la negociación de `Accept` y el `Vary`, **ejecutando el proxy**, porque una cabecera no está en el prerender; `robots()` en sus **dos** entornos, porque el `robots.txt` que se construye en CI es el de no producción y leerlo daría por bueno un `Disallow`; y las cabeceras, los alias y el **catálogo ARD**, contra la **regex ya compilada** de `routes-manifest.json` y el cuerpo prerenderizado de `/.well-known/ard.json`, que es lo que el servidor sirve de verdad. Lo primero que deja fuera, a propósito, es la nota de los escáneres de agentes: su puntuación mezcla lo que aplica con lo que no, y sus reglas son borradores que cambian sin avisar (`D159`, `D160`) |
| `check:deuda` | Que la deuda que mide Qlty vuelva a crecer. Es un **trinquete**, no un objetivo: no persigue una cifra —hay funciones complejas porque el problema lo es, y partirlas para bajar un contador las empeora—, prohíbe empeorar. Existe por una medida: en dos días pasó de 51 hallazgos a 71 y los **veinte** de diferencia estaban todos en `scripts/`, así que no había un problema de stock sino de crecimiento. El sello **cuadra exacto**, y por debajo también sale rojo: la forma de desactivar un trinquete no es tocar su script, es inflar su número (`D186`). Y sella la **magnitud** de cada hallazgo además de su presencia, que es lo que cierra el hueco de mirar solo el recuento: dentro de algo ya marcado, la complejidad podía crecer sin que el contador se moviera (`D196`) |
...
> **un `git push` se aborta** si uno de los gates que corre está en rojo
...
npm run check:capturas      # que las tres capturas del README sigan enseñando frases
                            # que el sitio dice hoy: titular, kicker, nav y stack (D194)
...
scripts/check-*.ts         Los guardianes de CI. Todos comparten dos reglas de método:
                           ...
                           El que crece guarda sus bloques en la carpeta hermana de su
                           nombre y el archivo suelto se queda con el orden en que corren
...
scripts/hooks/             Los que se disparan solos. Dos al EDITAR (Prettier y el guardián
                           ...
                           bloquea: lo que ese hook mira no llega a CI en rojo (D204, D207). Cuáles
                           son se leen en `regeneradores.mjs`; el de push lo instala
                           `core.hooksPath`
