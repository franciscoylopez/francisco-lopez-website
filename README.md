...
- **Analítica y consentimiento**: Google Tag Manager + GA4 y **Microsoft Clarity**, con **Consent Mode v2** y banner de consentimiento granular (RGPD), con página propia de privacidad y cookies. **Sin aceptar no sale ninguna petición a Google ni a Microsoft** (`D214`): el contenedor de GTM, y con él GA4 y Clarity, solo se inyecta con consentimiento de analítica. Hasta el 2026-09-17 no era así, porque `analytics_storage: denied` gobierna el almacenamiento y no el envío (`D198`), y GA4 recibía un `page_view` sin cookies de todo el mundo; la serie de GA4 se parte ese día. Lo que mide sin preguntar son **dos cosas, las dos declaradas en `/cookies`**: el contador de consentimiento (`D169`), tres enteros que no son dato personal; y **Vercel Web Analytics** (`D170`), que sí lo trata y es lo único que ve al que no consiente. Es una excepción al criterio propio de este sitio, no a la norma, y así está escrita.
