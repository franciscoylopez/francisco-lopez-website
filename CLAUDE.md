...
- **Un `fixed` que apila N elementos se acota al hueco que le queda** (`top`/`bottom` + scroll interno, centrado con `my-auto`), nunca se centra sobre el viewport: su alto crece con el contenido y el viewport no. *(P68.57: el capítulo doce metió el riel de secciones debajo del nav.)*
...
3. **¿Es un widget con estado, foco atrapado o portal** (diálogo, popover, tooltip, combobox, menú, tabs, scroll-area)? → **el teclado, el ARIA y el foco no se escriben a mano**, y salen de dos preguntas en este orden *(2026-08-24, D6)*: **¿lo trae la plataforma?** —`<dialog>` + `showModal()`, el atributo `popover`, `anchor-name`— y, si no, **¿lo trae shadcn?**, con `npx shadcn@latest add <componente>` (estilo `base-nova`, ya configurado) y nuestros tokens encima. *(Misma forma que la regla de iconos: «¿lucide lo trae? → no se dibuja».)* **Aplica hacia delante, no hacia atrás:** los widgets que hoy están a mano —el `<dialog>` nativo del consentimiento y su switch, las pestañas del Toolkit y los tabs de dispositivo del Design System— están bien hechos y no tienen deuda de accesibilidad; no se reescriben por cumplir la regla. Ver `DECISIONS.md` D6 y D120.
...
| 11 | **Figura con lienzo escalado**: rótulo ≥11px **pintados a 360**, que es el suelo declarado del rótulo de figura (D124) | Lo pone la capa (`DosLienzos`: se declara el ancho del lienzo y nada más, D114) y `npm run check:figuras` lo confirma en CI. Y su `/prototype` se ve a 360, no solo a escritorio (P68.56) |
