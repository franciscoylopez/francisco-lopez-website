...
- D36 · Capa de componentes: las cuatro capas (`action.tsx` · `layout.ts` · `heading.tsx` · `badge.tsx`); frontera `ui/`↔`site/` (¿la pieza sabe algo de este sitio?) y, dentro, ¿se pulsa?
...
1. **¿Existe ya la pieza?** — `components/ui/action.tsx` para todo lo **accionable**, `components/ui/badge.tsx` para el **rótulo que no se pulsa** (`tone` × `kind`), `components/ui/heading.tsx` para el par **eyebrow + titular** (`SectionHeader`), `components/ui/layout.ts` para **cajas y ritmos** (`WRAP`/`SECTION`/`CARD`/`PANEL`/`PAIR`), el resto de primitivas de `components/ui/` (`icons`, `rich`, `info-card`), los bloques de `components/site/`. → **Se usa.** No se replica su aspecto con clases sueltas. **Entre acción y etiqueta decide una pregunta: ¿se pulsa?** — un chip que solo rotula no es un botón pequeño (D36). **Dónde va lo nuevo:** ¿la pieza sabe algo de ESTE sitio (copy, rutas, datos, secciones)? No → `ui/`. Sí → `site/` (D36).
