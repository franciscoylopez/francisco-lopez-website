...
- D36 · Capa de componentes: las SIETE capas (`action.tsx` · `chrome.tsx` · `badge.tsx` · `heading.tsx` · `table.tsx` · `stat-row.tsx` · `layout.ts`; la séptima entró el 2026-08-19 con D64); frontera `ui/`↔`site/` (¿la pieza sabe algo de este sitio?) y, dentro, ¿se pulsa? → ¿tiene caja?
...
- D51 · Una herramienta externa entra por el **trabajo que resuelve**, no por lo buena que sea: el criterio en cuatro preguntas, y el patrón que salió repetido en tres bloques —**lo que encaja es de cero JS de cliente y se ata a un evento; lo que peor encaja añade cliente o una segunda fuente de reglas**—; de 32 candidatas entran 4; `agent-browser` mide lo que la pestaña oculta no puede (D50 reproducible, axe, vitals, dos temas) y **se validó reproduciendo dos resultados conocidos**; su límite **no** es la navegación inicial sino el sandbox entero —bajo él ningún comando llega al daemon; se conduce con el sandbox desactivado (corregido el 2026-08-17)—; y `.claude/skills/` está rastreado por git, así que lo de terceros va global. **Ampliado (2026-08-18): el primer descarte MEDIDO** — `graphify` se descarta, y no por el precio: su AST avisó de «30 source files produced zero nodes» y eran los **diccionarios JSON**, o sea ciega justo donde ocurrieron D57/D58/D60; de paso dejó la quinta aparición del metro que aprueba sobre lista vacía (`prettier --check` sobre rutas ignoradas) y la regla de método que aguantó: **no se construye el grafo con la mitad de los datos**, porque contestaría «no hay drift» por ausencia de datos
...
- D53 · La plantilla del deep-dive: una forma para cinco páginas y **el tipo como guardián** — el diccionario se declara con INTERFAZ explícita y no con `typeof` (cinco archivos que comparten forma), `EXPERIENCES` pasa a `as const satisfies` para que el slug sea unión real, y quitar el `Partial` del registro hace que una experiencia sin diccionario rompa el build; `sub`/`level 3` en `heading.tsx`, `parents` en `PageShell` (primer breadcrumb de tres niveles) y `ui/page-closer.tsx` (el cierre de página sube ENTERO, no solo la tarjeta); y lo que cazó `gate:html`: `cn()` reordena las clases en runtime. **Ampliado (2026-08-17):** el bloque de historia gana `imagen`/`video`/`cierre`; la captura va AL LADO porque un artefacto se LEE y una captura se RECONOCE; y el corte de `cierre` es **de contenido, no de maquetación**
- D54 · **Un artefacto se enseña, no se recrea**: se descarta el redibujo con tokens (cumplía la letra de la política e incumplía su espíritu) y se publica el render REAL de Mermaid, saneado por `scripts/artefacto-svg.ts` —fuera la hoja de estilos a cdnjs, la paleta fija y el pan/zoom del editor—; inline y no `<img>` porque una imagen no ve las variables CSS; el artefacto NO se traduce; el techo de 8 nodos se rompe a propósito; y los dos fallos del traductor (bbox sin acumular `translate`, y cambiar la fuente descuadra los anchos que Mermaid ya calculó). **Ampliado (2026-08-17):** «se remapea a tokens» era falso para el residuo —17 literales y 5 slabs blancos que no conmutaban—, y el arreglo no es ampliar la lista sino un **guardián que busca la AUSENCIA de literales** (giro de D38), validado disparándolo. **Ampliado (2026-08-18):** el render deja de ser manual (`npm run artefacto`, mermaid-cli con `channel: "chrome"` — sin descargar Chromium, local y determinista) porque el paso a mano publicó el diagrama SIN sanear; eso destapó que `cajaDelGrafo` pisaba un `viewBox` mejor que el suyo (3.070×2.692 frente a 2.192×1.742, el grafo en dos tercios de su lienzo) — **el `viewBox` es de las pocas cosas de un SVG que no se pueden verificar sin VERLO**; y el ancho de panel gana al 1:1 con scroll, decidido viéndolo
- D55 · **Un vídeo de terceros entra con facade, y el clic es el gate**: sin iframe en el DOM hasta pulsar, póster auto-hospedado (tirar de `i.ytimg.com` haría la petición que el facade evita), `frame-src` a `youtube-nocookie` (2.ª ampliación de la CSP tras D32) y sección propia en la política de cookies, fuera de la tabla; el clic es más estricto que gatearlo por categoría; y **un control de color fijo sobre un fondo arbitrario no llega al 3:1** — velo de `--background` al 0,35 (medido, no elegido) + anillo, con sus dos lecciones de método (el modelo daba 3,56 donde la pantalla daba 2,81; el muestreo del anillo caía sobre el triángulo)
...
- D59 · **El SEO del deep-dive, y las tres listas de páginas escritas a mano** (sitemap, `llms.txt` y la tabla de tarjetas OG): las seis se DERIVAN de `EXPERIENCES`; el inglés gana entrada propia en el sitemap (de 6 a 24 URLs) y el `x-default` entra donde ya estaba en el `<head>`; `lastModified` deja de ser `new Date()` —Google ignora el lastmod poco fiable, así que la señal **se estaba tirando**— y es declarado porque **Vercel clona en superficial** y una fecha derivada de un historial que no está es un hueco; `WebPage` y no `Article` (marcar cinco páginas de carrera como artículos dice «blog»), atado al `Person` por `@id`, **sin `isPartOf`** porque ese nodo no existe y una referencia colgante valida igual y miente; la tarjeta OG por experiencia destapó un solape **preexistente** del titular con los flancos que solo se ve renderizando; y el peso del deep-dive **no era el problema** (223 KB, 94/100 móvil — el Design System pesa 341 y saca lo mismo: la nota la fija el render delay, no los KB)
- D60 · **Una fuente única evita dos verdades; no mantiene al día una copia impresa**: los PDFs del CV son un artefacto COMMITEADO y se quedaron viejos en silencio al cambiar un sector — familia de fallo distinta a D38/D44/D57 (una sola escritura, pero una copia derivada que no se recalcula); `npm run check:cv` sella la **huella de las ENTRADAS** y no bytes, porque **el PDF no es determinista** (medido); validado rompiéndolo y comprobando el código de salida; **no cubre** los estilos del generador, a propósito; el tercer hueco —atar la narrativa a los bullets— **se descartó MIDIENDO** (INDYA daría 5 falsos positivos de 5, porque el formato lo permite); y las **skills caducan peor que los `.md`** porque se siguen en vez de leerse: `update-cv` acumuló nueve afirmaciones falsas en un solo día
- D61 · **Una superficie también cambia por ESTADO** (tercer eje de D39, que solo miraba clase y atributo): `hover:bg-muted` compila a `.hover\:bg-muted:hover` dentro de `@media (hover: hover)` —OTRO selector—, así que el cierre de las doce páginas cambiaba de fondo sin recalcular su atenuado; medido 9,14 en reposo → **7,79 claro / 9,01 oscuro** en hover donde debía dar 8,17/9,17; se arregla en la capa y **no** en los dos call sites porque `data-surface` es estático y no puede describir algo que cambia con el estado; era un **par 14 sin publicar** que al arreglar la causa se colapsa en uno que ya estaba en la tabla; y lo encontró preguntarse por qué el censo decía «8 páginas» de doce — *un metro bien calibrado que no se pasa por todo el sitio sigue siendo un metro que no ha mirado*
- D62 · **El 404 de una ruta que CASA no lo cubre `global-not-found`**: su contrato son las URLs que no casan con NINGUNA ruta, y `/trayectoria/loquesea` sí casa — diez rutas servían el 404 pelado de Next en producción; `dynamicParams = false` mueve el rechazo al enrutado sin perder el prerenderizado; y el 200 de `/trayectoria/EMENDU` en local era el sistema de archivos de Windows, no una URL duplicada
- D63 · **La raya no era un reemplazo, eran tres familias**: ~90 parentéticos que se REESCRIBEN, ~208 incisos y separadores, 8 rangos de fecha donde el `·` no vale, y dos excepciones que se quedan (el ordinal de D43 y la celda «no aplica»); el EN había derivado a la raya donde su gemelo ES ya usaba `:`; y `check:raya` en CI busca la AUSENCIA y afirma cuánto ha mirado — con las dos trampas de su validación (`$?` detrás de un pipe, y fallar por stack trace en vez de por mensaje) y el punto ciego de `.prettierignore` que se tragó el propio guardián
- D64 · **Una apertura homogénea no la decide el anclaje: la deciden los altos**: el pliegue sube a tres páginas, pero la homogeneidad costó tres pasadas — el eyebrow lo movía el `items-center` con una ilustración más alta que el texto (venía de antes), la fila de datos la decidía la ilustración mientras mandara, y se arregla con `self-start`, compactando las composiciones (compactar ≠ encoger) y `HERO_ROW` con `min-h` en la capa; **si un anclaje «arregla» una inconsistencia, probablemente esté tapando la causa**; incluye el cuerpo de Cookies fuera de `PROSE`, su exclusión MEDIDA del pliegue, el `[&_p]:m-0` que mataba dos márgenes por especificidad, y la séptima pieza de la capa (`stat-row.tsx`)
- D65 · **Un vídeo de apertura no es una foto que se mueve**: no puede llevar `loop`, no termina en el encuadre de la foto que sustituye, a 1280×618 no cabe entero con ningún anclaje (aritmética), y `autoPlay` + `preload="none"` NO evita la descarga con motion reducido —medido: 362 KB para ver una imagen de 17— así que lo arranca un script inline gateado por la media query; el scrim mejora a 6,27/10,85 midiendo el fondo SIN el texto
- D66 · **Un asset tiene más consumidores de los que se ven**: la foto del hero alimenta el hero, la tarjeta OG y el `image` del JSON-LD —el tercero no se ve mirando la página y quedaba en 404 al renombrar—, y del archivo OG de 1200×630 solo se servían los 600 centrales porque `/api/og` lo mete en una caja de 600×630
- D67 · **El ruido conocido de los validadores se documenta por MECANISMO, no por cifra**: de ~11 hallazgos externos, SEIS eran falsos positivos —las reglas `@property` de Tailwind v4 que el W3C no implementa, la barra final de los void elements que emite `react-dom/server` (nivel INFO, y los atributos SÍ van entrecomillados: 1.392 a 0), la relación texto/HTML que Google no usa, y los assets del challenge de Vercel—; el quinto (imágenes sin `alt`, enlaces sin `title`) es el que ya se reabrió DOS veces y queda medido sobre las 24 variantes (142 `<img>`, cero sin `alt`; 502 `<a>`, cero sin nombre accesible; y `title` no es requisito de WCAG sino antipatrón); el sexto —`unsafe-inline`— es REAL y ya estaba aceptado en D26, con su condición de revisión. **La forma la impuso verificarlo**: TODAS las cifras apuntadas estaban viejas (79→69 `@property`, «7 imágenes»→142), así que se documenta el mecanismo y el comando para recontar, nunca el número — y el propio recuento trae su falso aprobado (la hoja cuelga de `/chunks/`, no de `/css/`)
- D68 · **El repositorio es público, y a `main` la protege el servidor y no la disciplina**: en plan Free un repo PRIVADO no admite protección de rama ni rulesets (403 en los dos caminos), así que la tarea no era configurar sino DECIDIR, y publicar es irreversible; la auditoría de los 293 commits salió limpia de secretos —los IDs de GTM y Clarity ni están en el repo— y **el riesgo real era editorial**: `PRD-Historical.md` registraba lo que se decidió no contar, o sea lo republicaba; el ruleset lleva 0 aprobaciones requeridas (trabajas solo), `calidad y build` como check, y `squash`/`rebase` = D12 escrita en el servidor, **sin bypass de admin**, validado con un push real rechazado por `GH013`; su efecto de segundo orden mordió —**cambiar el nombre de un check requerido invalida en silencio todo PR abierto**, y dejó las cinco actualizaciones de Dependabot bloqueadas—; y con el repo público llegan el README de portada, el `LICENSE` («público para consulta, no código abierto») y el enlace del footer con el segundo icono propio
...
> - **`General` significa TRANSVERSAL, no «no sé dónde ponerla».** Higiene de CI, dependencias, docs y proceso: cosas que no son de ninguna página. Si una tarea es de una página o de una capa concreta, va a su bloque. *(Vigilado desde el 2026-08-19: el eje ya degeneró una vez con «Optimización» al 80%, y al revisar «General» al 43% resultó que 17 de 20 SÍ eran transversales. El riesgo no era el volumen: era que nadie hubiera escrito el criterio.)*
